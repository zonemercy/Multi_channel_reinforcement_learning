...
P_e = 1-ZunPhyModel(sigma_s, L_payload, 19.2*10^3);
...
for lambda = lvec,
    ...
    P_idle(counter) = p(1);
    % Frame transmission failure probability (due to collisions and channel errors): 
    ...
    E_inst(counter) = ((P_idle(counter) * 0.8 + (1 - P_idle(counter))*(Reliability(counter) * 80 + Pcf(counter) * 80 * 2 + Pcr(counter) * 80 * macMaxCSMABackoffs)))...
        / (Reliability(counter) * S_inst(counter));
    E_avg(counter) = ((P_idle(counter) * 0.8 + (1 - P_idle(counter))*(Reliability(counter) * 80 + Pcf(counter) * 80 * 2 + Pcr(counter) * 80 * macMaxCSMABackoffs)))...
        / S_avg(counter);
    % No p_sleeping
    P_sleeping(counter) = P_idle(counter) + (1 - P_idle(counter)) * (Reliability(counter) + Pcf(counter) + Pcr(counter));
...
hold on,
grid on,
plot(lvec*L_application,E_inst(cnt),'-xr');
hold on,
plot(lvec*L_application,E_avg(cnt),'-xb');
% hold on,
% plot(lvec*L_application, Reliability(cnt), '-g');
title( ['Average Energy VS Offered Load for ',num2str(N_stations),' Nodes']);
xlabel('Offered Load (bits/application/node)');
ylabel('Average Energy (mW)');

% figure,
% hold on,
% grid on,
% plot(lvec*L_application,Pcf(cnt),'-xr');
% hold on,
% plot(lvec*L_application,Pcr(cnt),'-or');
% hold on,
% plot(lvec*L_application,P_sleeping(cnt),'-xg');
% hold on,
% plot(lvec*L_application, Reliability(cnt), '-xb');
% hold on,
% plot(lvec*L_application, P_idle(cnt), '-xm');
% hold on,
% plot(lvec*L_application, P_blocking(cnt), '-xk');
% hold on,
% plot(lvec*L_application, P_failure(cnt), '-xc');
% title( ['p',num2str(N_stations),' Nodes']);
% legend('Pcf','Pcr','P_sleeping','Reliability','P_idle','P_blocking','P_failure');
% xlabel('Offered Load (bits/application/node)');
% ylabel('P');
