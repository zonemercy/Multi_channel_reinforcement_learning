...
N_stations = 5;
% lnode = 5:5:40;
lnode = [5,8,10,12,15,18,20];
D_node = zeros(size(lnode));
R_node = zeros(size(lnode));
PF_node = zeros(size(lnode));
E_node = zeros(size(lnode));
PFbackoff_node = zeros(size(lnode));
node_counter = 1;
for N_stations = lnode,
...
%     P_e = 1-ZunPhyModel(sigma_s, L_payload, data_rate);
    ...
    P_f(counter) = P_failure(counter) + Pcf(counter) + Pcr(counter);
...
%    omega_high = omega_low + 0.25;
%    omega_figure(counter) = omega_low; %fuzzy logic
%    
%     if p(1) < omega_low,
%       if n_c < 4,
%            n_c = n_c + 1;
...
D_node(node_counter) = mean(D_value);
PF_node(node_counter) = mean(P_failure);
PFbackoff_node(node_counter) = mean(Pcf);
R_node(node_counter) = mean(Reliability);
E_node(node_counter) = mean(ESB);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
cnt = 1:node_counter-1;
% figure,
hold on,
grid on, 
cmap = ['r','g','b','c','m','k'];
crand = cmap(randi([1,length(cmap)],1,1));
plot(cnt,E_node(cnt),'-s','Color',crand,'LineWidth',1);
% plot(cnt,PF_node(cnt),'-s','Color',crand,'LineWidth',1);
% plot(cnt,PF_node(cnt),'-s','Color',crand,'LineWidth',1);
% plot(cnt,PFbackoff_node(cnt),'-s','Color',crand,'LineWidth',1);
...
ylabel('ESB');
%csvwrite('/Users/zonemercy/R/energy1.csv',ESB')
...
% bb=reshape(aa,8,10);
% cc=mean(bb);
%csvwrite('/Users/zonemercy/R/energy1.csv',lnode)
%dlmwrite('/Users/zonemercy/Documents/MATLAB/energy_node.csv',ESB,'-append')
%dlmwrite('/Users/zonemercy/Documents/MATLAB/energy_node.csv',E_node,'-append')
