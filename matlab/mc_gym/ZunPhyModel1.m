...
function p =ZunPhyModel1(sigma, PACKET_LENGTH, DATA_RATE, d1, ShadowingStd,path_loss)
...
p_temp=dblquad(@(d1, ShadowingStd) Calc(d1, distmin, distmax, noiseFloor,DATA_RATE,...
...
p_temp=p_temp/((distmax-distmin)*4*SHADOWING_STANDARD_DEVIATION);
p=Calc(d1, distmin, distmax, noiseFloor,DATA_RATE,...
    BW, PATH_LOSS_EXPONENT, lambda, ShadowingStd, outputPower, NOISE, D0, PREAMBLE_LENGTH,FRAME_LENGTH,S);
% fprintf('%d',lambda);
p_quadl=quadl(@(ShadowingStd) Calc(d1, distmin, distmax, noiseFloor,DATA_RATE,...
    BW, PATH_LOSS_EXPONENT, lambda, ShadowingStd, outputPower, NOISE, D0, PREAMBLE_LENGTH,FRAME_LENGTH,S),...
    -2*SHADOWING_STANDARD_DEVIATION, 2*SHADOWING_STANDARD_DEVIATION);
p_quadl=p_quadl/(4*SHADOWING_STANDARD_DEVIATION);
fprintf('%f, %f, %f',p,p_quadl,p_temp);
