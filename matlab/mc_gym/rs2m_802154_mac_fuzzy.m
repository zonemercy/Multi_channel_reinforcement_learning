...
for lambda = 8.2-abs(8-lvec),
    ...
    pr_mean(1)=0.3;
    pr_mean(counter) = 0.3*P_failure(counter)+0.7*pr_mean(max(1,(counter-1)));
...
%     ET_mean(counter) = 0.1*D_value(counter)+0.9*ET_mean(max(1,(counter-1)));,'g','b','c','m','k'
    ...
    ESB(counter) = (P_idle(counter) * E_idle*n_c + Reliability(counter) * E_R(counter) + ...
        Pcf(counter) * E_caf(counter) + Pcr(counter) * E_rtx(counter)) / S_avg(counter);
    E_f(counter) = Pcf(counter) * E_caf(counter) + Pcr(counter) * E_rtx(counter);
    ESB_mean(1)=1;
...
% dlmwrite('/Users/zonemercy/Documents/MATLAB/pr_mean.csv',pr_mean,'-append')
% BB=reshape(ESB,4,19)
% ESB(:,76)=[]
aa=[ESB,1,1];
bb=reshape(aa,8,10);
