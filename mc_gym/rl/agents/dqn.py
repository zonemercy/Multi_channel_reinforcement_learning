...
clf_filename = 'knn.pkl'
pre_clf = joblib.load(clf_filename)
...
class DQNAgent(AbstractDQNAgent):
    ...
    def forward(self, observation):
        # Select an action.
        state = self.memory.get_recent_state(observation[:-4])
        q_values = self.compute_q_values(state)

        # ob_list = observation[-4:]
        if np.std(q_values) < 0.3 and np.random.random(1)[0]<0.1:
            ob_list = [ round(elm, 0) for elm in observation[-4:] ]
            pre_proba = pre_clf.predict_proba(np.array(ob_list).reshape((1, -1)))[0]  
            q_values = np.array(map(max, zip(*[q_values,pre_proba])))
        ...
        # Book-keeping.
        self.recent_observation = observation[:-4]
